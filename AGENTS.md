...
- Tests stay offline: `AGENT_RUNTIME=scripted` (or `flue-echo` for the Pi harness), `SANDBOX_PROVIDER=fake`, in-process wakeup — no live OpenRouter / Cloudflare Computer / E2B.
...
- Desktop sandbox is owner opt-in on a trusted machine. Never enable it when the provider is `cloudflare` or `e2b`.
