...
- The Pi/executor must not import `fs`, `dockerode`, or Cloudflare bindings. The API Worker uses `@grogbot/adapters/edge` (scripted / gateway). Flue Node (`@flue/runtime/node`) and `apps/worker` are a later self-host path — do not add features that only work there.
